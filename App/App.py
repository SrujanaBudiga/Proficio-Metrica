...
def run():
    ...
    link = '<b>Built with 🤍 by <a href="https://dnoobnerd.netlify.app/" style="text-decoration: none; color: #021659;">Srujana</a></b>' 
    ...
    ###### CODE FOR ABOUT PAGE ######
        ...
        st.subheader("**About The Tool - PROFICIO METRICA**")
        ...
        <p align="justify">
            Built with 🤍 by 
            <a href="https://www.linkedin.com/in/srujana-b-6113a9213" style="text-decoration: none; color: blue;">Srujana</a>
        </p>
    ...
    ###### CODE FOR ADMIN SIDE (ADMIN) ######
        ...
        if st.button('Login'):
            ...
            if ad_user == 'admin' and ad_password == 'admin@resume-analyzer':
                ...
                st.success("Welcome Srujana ! Total %d " % values + " User's Have Used Our Tool : )")                
                ...
                values = pd.DataFrame({'Predicted_Field': value_counts.index, 'Count': value_counts.values})

                # Pie chart for predicted field recommendations
                st.subheader("**Pie-Chart for Predicted Field Recommendation**")
                fig = px.pie(values, values='Count', names='Predicted_Field', title='Predicted Field according to the Skills ', color_discrete_sequence=px.colors.sequential.Aggrnyl_r)
                st.plotly_chart(fig)

                # Fetching User_Level from the query and getting the unique values and total value count                 
                labels = plot_data.User_Level.unique()
                value_counts = plot_data.User_Level.value_counts()
                values = pd.DataFrame({'User_Level': value_counts.index, 'Count': value_counts.values})

                # Pie chart for User's Experienced Level
                st.subheader("**Pie-Chart for User's Experienced Level**")
                fig = px.pie(values, values='Count', names='User_Level', title="Pie-Chart 📈 for User's 👨‍💻 Experienced Level", color_discrete_sequence=px.colors.sequential.RdBu)
                st.plotly_chart(fig)


                # Fetching resume_score from the query and getting the unique values and total value count                 
                labels = plot_data.resume_score.unique()                
                value_counts = plot_data.resume_score.value_counts()
                values = pd.DataFrame({'resume_score': value_counts.index, 'Count': value_counts.values})

                # Pie chart for Resume Score
                st.subheader("**Pie-Chart for Resume Score**")
                fig = px.pie(values, values='Count', names='resume_score', title='From 1 to 100 💯', color_discrete_sequence=px.colors.sequential.Agsunset)
                st.plotly_chart(fig)

                # Fetching IP_add from the query and getting the unique values and total value count 
                labels = plot_data.IP_add.unique()
                value_counts = plot_data.IP_add.value_counts()
                values = pd.DataFrame({'IP_add': value_counts.index, 'Count': value_counts.values})

                # Pie chart for Users
                st.subheader("**Pie-Chart for Users App Used Count**")
                fig = px.pie(values, values='Count', names='IP_add', title='Usage Based On IP Address 👥', color_discrete_sequence=px.colors.sequential.matter_r)
                st.plotly_chart(fig)

                # Fetching City from the query and getting the unique values and total value count 
                labels = plot_data.City.unique()
                value_counts = plot_data.City.value_counts()
                values = pd.DataFrame({'City': value_counts.index, 'Count': value_counts.values})

                # Pie chart for City
                st.subheader("**Pie-Chart for City**")
                fig = px.pie(values, values='Count', names='City', title='Usage Based On City 🌆', color_discrete_sequence=px.colors.sequential.Jet)
                st.plotly_chart(fig)

                # Fetching State from the query and getting the unique values and total value count 
                labels = plot_data.State.unique()
                value_counts = plot_data.State.value_counts()
                values = pd.DataFrame({'State': value_counts.index, 'Count': value_counts.values})

                # Pie chart for State
                st.subheader("**Pie-Chart for State**")
                fig = px.pie(values, values='Count', names='State', title='Usage Based on State 🚉', color_discrete_sequence=px.colors.sequential.PuBu_r)
                st.plotly_chart(fig)

                # Fetching Country from the query and getting the unique values and total value count 
                labels = plot_data.Country.unique()
                value_counts = plot_data.Country.value_counts()
                values = pd.DataFrame({'Country': value_counts.index, 'Count': value_counts.values})

                # Pie chart for Country
                st.subheader("**Pie-Chart for Country**")
                fig = px.pie(values, values='Count', names='Country', title='Usage Based on Country 🌏', color_discrete_sequence=px.colors.sequential.Purpor_r)
                st.plotly_chart(fig)
